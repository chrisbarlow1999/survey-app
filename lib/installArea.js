...
// --- Draft support (see lib/formDraft.js) -----------------------------------
// Proof photos can't be serialised, so a restored draft has the areas and
// screens but every photo slot is empty again.
export function installAreaToDraft(area) {
  return {
    ...area,
    screens: (area.screens || []).map(({ photoFile, photoPreview, ...s }) => s),
  };
}

export function installAreaFromDraft(area) {
  const screens = (area.screens || []).map((s) => ({
    ...freshInstallScreen(),
    ...s,
    photoFile: null,
    photoPreview: null,
  ...
  return {
    ...freshInstallArea(),
    ...area,
    screens: screens.length ? screens : [freshInstallScreen()],
  };
}
