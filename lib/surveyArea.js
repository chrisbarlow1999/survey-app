...
// Photos can't be serialised, so they're dropped on the way into a draft and
// come back empty. Everything else about an area survives.
export function areaToDraft(area) {
  const { photoFile, photoPreview, additionalPhotos, ...rest } = area;
  return rest;
}

export function areaFromDraft(area) {
  return {
    ...freshArea(),
    ...area,
    photoFile: null,
